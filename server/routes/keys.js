...
const createKeyValidation = [
  ...
  body('rateLimit').optional().isInt({ min: 1, max: 100000 }),
  body('rateLimitWindow').optional().isInt({ min: 60, max: 86400 }),
...
const generateApiKey = () => {
  const prefix = 'ag'; // API Guardian prefix
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    let query = `
      SELECT ak.*, a.name as api_name, a.base_url as api_base_url,
...
router.post('/', authenticateToken, createKeyValidation, async (req, res) => {
  try {
    ...
    const keyResult = await pool.query(keyQuery, [
      apiId,
      userId,
      apiKey, // Store the full key (in production, this should be hashed)
      keyPrefix,
      name,
      description || null,
...
router.put('/:id', authenticateToken, checkResourceOwnership('api_key'), updateKeyValidation, async (req, res) => {
  try {
    ...
    const updateQuery = `
      ...
      FROM api_keys ak
      JOIN apis a ON ak.api_id = a.id
      WHERE ak.id = $1 AND ak.user_id = $${updateKeys.length + 2}
    `;
    ...
    if (updateFields.status) {
      try {
        ...
        await deleteCache(`api_key:${key.key_hash}`);
      } catch (redisError) {
...
router.post('/:id/regenerate', authenticateToken, checkResourceOwnership('api_key'), async (req, res) => {
  try {
    ...
    const newKeyPrefix = newApiKey.substring(0, 10) + '...';
    ...
    const updateQuery = `
      ...
      FROM api_keys ak
      JOIN apis a ON ak.api_id = a.id
      WHERE ak.id = $3 AND ak.user_id = $4
    `;

    const result = await pool.query(updateQuery, [newApiKey, newKeyPrefix, keyId, userId]);
    ...
    // Clear old cached key (the old key hash is no longer valid)
    try {
      const { deleteCache } = require('../config/redis');
      // We can't clear the old cache since we don't have the old key, but it will expire naturally
    } catch (redisError) {
      logger.error('Failed to clear key cache:', redisError);
    }
    ...
    res.json({
      ...
      data: {
        key: {
          id: key.id,
          name: key.name,
          apiKey: newApiKey, // Only returned once during regeneration
          keyPrefix: key.key_prefix,
          updatedAt: key.updated_at
        }
      }
...
router.delete('/:id', authenticateToken, checkResourceOwnership('api_key'), async (req, res) => {
  try {
    ...
    try {
      const { deleteCache } = require('../config/redis');
      await deleteCache(`api_key:${keyData.key_hash}`);
    } catch (redisError) {
      logger.error('Failed to clear key cache:', redisError);
    }
...
router.get('/:id/usage', authenticateToken, checkResourceOwnership('api_key'), async (req, res) => {
  try {
    ...
    const usageQuery = `
      ...
      FROM api_usage_logs 
      WHERE api_key_id = $1 AND created_at >= CURRENT_TIMESTAMP - INTERVAL '${interval}'
      GROUP BY ${dateFormat}
      ORDER BY period DESC
    `;

    const usageResult = await pool.query(usageQuery, [keyId]);
    ...
    const endpointResult = await pool.query(endpointQuery, [keyId]);
    ...
    const errorResult = await pool.query(errorQuery, [keyId]);
