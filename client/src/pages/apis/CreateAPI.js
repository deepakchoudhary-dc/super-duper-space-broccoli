...
const CreateAPI = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      const response = await apiAPI.create(formData);
      toast.success('API created successfully');
      navigate(`/apis/${response.data.data.api.id}`);
    } catch (error) {
            ...
            <Card>
              <CardContent>
                ...
                <Typography variant="body2" color="text.secondary" paragraph>
                  Follow our API registration guide to ensure proper setup and configuration.
                </Typography>
                <Button
                  variant="outlined"
                  size="small"
